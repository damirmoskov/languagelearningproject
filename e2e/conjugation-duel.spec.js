...
test.describe('Conjugation Duel Game', () => {
  ...
  test.beforeAll(() => {
    const dataPath = path.join(__dirname, '..', 'data', 'french_a2_verbs.json');
  ...
  test('player can win a round against the AI', async ({ page }) => {
    ...
    const verbInfo = verbsData.verbs.find(v => v.verb === verbInfinitive);
    ...
    await page.click('#cd-player-input'); // Ensure the input has focus before typing
    await page.keyboard.press('Enter');

    await expect(page.locator('#cd-feedback')).toContainText('You win!');
    ...
    await page.waitForTimeout(2500); // Wait for the next round delay
